...
class Menu(tk.Tk):
    ...
    def createWidgets(self):
        ...
        self.loginLabel = tk.LabelFrame(self,text='User login')
        self.loginLabel.pack()
        self.userName = Entry(self.loginLabel,textvariable=self.userNameEntry)
        self.userName.insert(0,'Username')
        self.password = Entry(self.loginLabel,show='*',textvariable=self.passwordEntry)
        self.password.insert(0,'Password')
        self.userName.pack()
        self.password.pack()
        self.userName.bind('<FocusIn>',self.tempUsernameText)
        self.password.bind('<FocusIn>',self.tempPasswordText)
        self.loginButton = ttk.Button(self.loginLabel,text='Login',command=self.verifyLogin)
        self.loginButton.pack()
        self.registerButton = ttk.Button(self.loginLabel,text='Create account',command=self.createUser)
